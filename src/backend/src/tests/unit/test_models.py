...
def test_model_create(dbsession):
    with closing(open(dataset, encoding='utf-8-sig')) as f:
        ...
        response = crud.create(dbsession, schemas.FakenamesCreate(**payload))
    assert response.guid == payload['guid']
...
def test_model_update(dbsession):
    with closing(open(dataset, encoding='utf-8-sig')) as f:
        ...
        response = crud.update(dbsession, schemas.FakenamesUpdate(**payload))
    assert response.guid == payload['guid']
...
def test_model_delete(dbsession):
    ...
    logger.info(f'''
                ...
                guid:      "{check.guid}"
                ''')
    response = crud.delete(dbsession, guid=check.guid)
    assert response.deleted is True
    assert response.guid == check.guid
