...
class FakenamesApis():
    ...
    # APIs
    @router.post('/v1/fakenames/postgres', response_model=schemas.Fakenames, status_code=201)
    ...
    @router.get('/v1/fakenames/postgres', response_model=List[schemas.Fakenames], status_code=200)
    def read_all(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
        """ Returns all fake identities """
        identities = crud.read_all(db=db, skip=skip, limit=limit)
        return identities

    @router.get('/v1/fakenames/postgres/{guid}', response_model=schemas.Fakenames, status_code=200)
    ...
    @router.patch('/v1/fakenames/postgres', response_model=schemas.Fakenames, status_code=200)
    def update(identity: schemas.FakenamesUpdate, db: Session = Depends(get_db)):
        """ Returns a fake identity based on its GUID """
        return crud.update(db=db, identity=identity)

    @router.delete('/v1/fakenames/postgres/{guid}', response_model=schemas.FakenamesDelete, status_code=200)
